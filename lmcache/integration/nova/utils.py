...
def create_lmcache_metadata(
    nova_config=None, model_config=None, parallel_config=None, cache_config=None
):
    """
    Create LMCacheEngineMetadata from Nova configuration.

    This function extracts common metadata creation logic that was duplicated
    across multiple files.

    Args:
        nova_config: Nova configuration object containing model, parallel, and
                    cache configs (alternative to individual config parameters)
        model_config: Model configuration (alternative to nova_config)
        parallel_config: Parallel configuration (alternative to nova_config)
        cache_config: Cache configuration (alternative to nova_config)
    ...
    # Support both nova_config object and individual config parameters
    if nova_config is not None:
        model_cfg = nova_config.model_config
        parallel_cfg = nova_config.parallel_config
        cache_cfg = nova_config.cache_config
    else:
