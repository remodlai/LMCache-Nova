...
from nova.core.distributed.kv_transfer.kv_connector.v1.base import (
    KVConnectorBase_V1,
    KVConnectorMetadata,
    KVConnectorRole,
)
from nova.core.distributed.parallel_state import (
    get_tensor_model_parallel_rank,
    get_tp_group,
)
from nova.core.sampling_params import SamplingParams

# First Party
# Use LMCache's own math utilities instead of nova's
# (avoids dependency on nova internal changes like https://github.com/vllm-project/vllm/pull/27188)
...
try:
    # Third Party
    from nova.core.utils.torch_utils import get_kv_cache_torch_dtype
except ImportError:
    # Third Party
    from nova.core.utils import get_kv_cache_torch_dtype

# Third Party
from nova.manager.core.scheduler.output import SchedulerOutput
from nova.version import __version__ as NOVA_VERSION
import torch

# First Party
from lmcache import utils
from lmcache.config import LMCacheEngineMetadata
from lmcache.integration.nova.utils import (
    ENGINE_NAME,
...
from lmcache.v1.gpu_connector import (
    ...
    NOVABufferLayerwiseGPUConnector,
    NOVAPagedMemGPUConnectorV2,
    NOVAPagedMemLayerwiseGPUConnector,
)
...
if TYPE_CHECKING:
    # Third Party
    from nova.core.attention.backends.abstract import AttentionMetadata
    from nova.core.forward_context import ForwardContext
    from nova.core.multimodal.inputs import PlaceholderRange
    from nova.manager.core.kv_cache_manager import KVCacheManager
    from nova.manager.core.scheduler.output import NewRequestData
    from nova.request import Request
...
@dataclass
class LoadSpec:
    # Number of tokens cached in Nova
    nova_cached_tokens: int
    ...
    def from_new_request(
        ...
        Args:
            ...
            num_tokens_to_compute (int): the number of tokens that will
                be 'computed', including the `num_computed_tokens` (Nova's
        ...
        """
        # Nova 0.9.0 update: request.block_ids changed from list[int] to
        # list[list[int]]
...
class ReqMeta:
    ...
    def from_request_tracker(
        ...
        Args:
            ...
            block_size (int): the block size in Nova.
...
def _calculate_draft_layers(nova_config, model_config):
    num_draft_layers = 0
    if nova_config is not None and nova_config.speculative_config is not None:
        logger.info(f"nova_config.speculative_config: {nova_config.speculative_config}")
        # TODO(baoloongmao): Support other MTP/draft methods
        if nova_config.speculative_config.method == "deepseek_mtp":
            num_draft_layers = getattr(
                model_config.hf_config, "num_nextn_predict_layers", 0
            )
        elif nova_config.speculative_config.use_eagle():
            try:
                draft_model_config = nova_config.speculative_config.draft_model_config
                num_draft_layers = draft_model_config.get_num_layers(
                    nova_config.parallel_config
                )
...
def _init_lmcache_engine(
    ...
    :param nova_config: The Nova configuration.
    :type nova_config: NovaConfig
    ...
    model_config = nova_config.model_config
    parallel_config = nova_config.parallel_config
    cache_config = nova_config.cache_config
    ...
    num_draft_layers = _calculate_draft_layers(nova_config, model_config)
    ...
    nova_gpu_connector: Optional[GPUConnectorInterface]
    ...
    if role == "scheduler":
        ...
        if lmcache_config.enable_blending:
            ...
            nova_gpu_connector = NOVABufferLayerwiseGPUConnector(
                hidden_dim_size,
                num_layer,
                use_gpu=use_gpu,
            ...
            nova_gpu_connector = NOVAPagedMemLayerwiseGPUConnector(
                hidden_dim_size,
                num_layer,
                use_gpu=use_gpu,
                chunk_size=chunk_size,
        ...
        nova_gpu_connector = NOVAPagedMemGPUConnectorV2(
            hidden_dim_size,
            num_layer,
            use_gpu=use_gpu,
            chunk_size=chunk_size,
...
class LMCacheConnectorV1Impl:
    def __init__(
        ...
        self.kv_role = nova_config.kv_transfer_config.kv_role
        self.worker_count = nova_config.parallel_config.tensor_parallel_size
        config = lmcache_get_or_create_config()
        assert isinstance(config, LMCacheEngineConfig), (
            "LMCache v1 configuration is should be passed for Nova v1."
        )
        # Put the leading with "lmcache." and matched configs from
        # nova extra_config to the config
        kv_connector_extra_config = (
            nova_config.kv_transfer_config.kv_connector_extra_config
        )
            ...
            for key, value in kv_connector_extra_config.items():
                if key.startswith("lmcache."):
                    ...
                    if _validate_and_set_config_value(config, config_key, value):
                        logger.info(
                            f"Updated config {config_key} from Nova "
        ...
        if role == KVConnectorRole.SCHEDULER:
            ...
            self.lookup_client = LookupClientFactory.create_lookup_client(
                nova_config, config, self.lmcache_engine
            )
            ...
            self.lookup_server = LookupClientFactory.create_lookup_server(
                self.lmcache_engine, nova_config
            )
        ...
        self._block_size = nova_config.cache_config.block_size

        # request_id -> (nova cached tokens, lmcache cached tokens)
        ...
        self._discard_partial_chunks = (
            nova_config.kv_transfer_config.get_from_extra_config(
        ...
        self.skip_last_n_tokens = nova_config.kv_transfer_config.get_from_extra_config(
            "skip_last_n_tokens", 0
        )

        self.num_layers = nova_config.model_config.get_num_layers(
            nova_config.parallel_config
        )
        ...
        if nova_config.parallel_config.data_parallel_rank_local == 0:
        ...
        logger.info(
            ...
            f"nova version {NOVA_VERSION}, "
            "lmcache cache_engine metadata: "
            f"{getattr(self.lmcache_engine, 'metadata', None)}"
        )

    def get_inference_info(self) -> dict:
        """Get inference information including Nova config and related details.

        Returns:
            dict: Dictionary containing inference information
        """
        # Get Nova config information
        nova_config = self._nova_config

        # Use Nova config's string representation and add specific configs
        inference_info = {
            "nova_version": NOVA_VERSION,
            "lmcache_version": utils.get_version(),
            "nova_config": str(nova_config),
            "model_config": {
                "model": getattr(nova_config.model_config, "model", None),
                "dtype": str(getattr(nova_config.model_config, "dtype", None)),
                "max_model_len": getattr(
                    nova_config.model_config, "max_model_len", None
                ),
                "vocab_size": getattr(nova_config.model_config, "vocab_size", None),
                "num_layers": getattr(
                    nova_config.model_config, "get_num_layers", lambda _: None
                )(nova_config.parallel_config),
                "num_attention_heads": getattr(
                    nova_config.model_config, "get_num_attention_heads", lambda _: None
                )(nova_config.parallel_config),
                "num_kv_heads": getattr(
                    nova_config.model_config, "get_num_kv_heads", lambda _: None
                )(nova_config.parallel_config),
                "head_size": getattr(
                    nova_config.model_config, "get_head_size", lambda: None
                )(),
            },
            "cache_config": {
                "block_size": getattr(nova_config.cache_config, "block_size", None),
                "cache_dtype": str(
                    getattr(nova_config.cache_config, "cache_dtype", None)
                ),
                "gpu_memory_utilization": getattr(
                    nova_config.cache_config, "gpu_memory_utilization", None
                ),
                "swap_space": getattr(nova_config.cache_config, "swap_space", None),
                "enable_prefix_caching": getattr(
                    nova_config.cache_config, "enable_prefix_caching", None
                ),
            },
        }
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs) -> None:
        """Start loading the KV cache from the connector buffer to Nova's
        paged KV buffer.
        ...
        for idx, request in enumerate(metadata.requests):
            ...
            masked_token_count = (
                request.load_spec.nova_cached_tokens
            ...
            if self.use_layerwise:
                ...
                num_expected_tokens = (
                    lmcache_cached_tokens - request.load_spec.nova_cached_tokens
                )
            ...
            self._stats_monitor.update_interval_nova_hit_tokens(
                request.load_spec.nova_cached_tokens
            )
    ...
    def record_failed_blocks(
        ...
        Args:
            request_id: request id from Nova.
            expected_mask: Boolean tensor indicating which tokens were expected to
                ...
                False means the token is already cached in Nova and does not need
        ...
        Example:
            expected_mask = [F, T, T, T] meaning the 1st is in Nova cache
    ...
    def wait_for_layer_load(self, layer_name: str) -> None:
        """Blocking until the KV for a specific layer is loaded into Nova's
        paged buffer.
    ...
    def save_kv_layer(
        ...
        **kwargs,
        ...
        """Start saving the a layer of KV cache from Nova's paged buffer
        to the connector.
    ...
    def get_num_new_matched_tokens(
        ...
        # This will be removed in the future if Nova's scheduler provides
        ...
        self.load_specs[request.request_id] = LoadSpec(
            nova_cached_tokens=num_computed_tokens,
        ...
        # TODO: Align to Nova block size. Should test whether it can be removed
    ...
    def update_state_after_alloc(self, request: "Request", num_external_tokens: int):
        ...
        if (
            ...
            assert (
                ...
                == self.load_specs[request.request_id].lmcache_cached_tokens
                - self.load_specs[request.request_id].nova_cached_tokens
            ), (
                f"Mismatch in number of tokens: {num_external_tokens} vs "
                f"{self.load_specs[request.request_id].lmcache_cached_tokens} - "
                f"{self.load_specs[request.request_id].nova_cached_tokens}"
    ...
    def build_connector_meta(
        ...
        # NOTE: For backward compatibility with nova version < 0.9.2,
        # In the latest nova version, the type of scheduled_cached_reqs has
