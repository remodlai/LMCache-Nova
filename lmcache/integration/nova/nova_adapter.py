...
if TYPE_CHECKING:
    from nova.core.worker.model_runner import ModelInputForGPUWithSamplingMetadata

# Third Party
from nova.core.attention import AttentionMetadata

# from nova.core.attention.backends.flash_attn import FlashAttentionMetadata
try:
    # Third Party
    from nova.core.attention.backends.flash_attn import FlashAttentionMetadata
except (ModuleNotFoundError, ImportError):
    # nova_flash_attn is not installed, try the ROCm FA metadata
    from nova.core.attention.backends.rocm_flash_attn import (
        ROCmFlashAttentionMetadata as FlashAttentionMetadata,
    )

# Third Party
from nova.core.attention.backends.flashmla import FlashMLAMetadata
from nova.core.attention.backends.mla.common import MLACommonMetadata
...
from nova.core.sequence import IntermediateTensors

# First Party
from lmcache.integration.nova.utils import ENGINE_NAME
from lmcache.logging import init_logger

# Use LMCache's own math utilities instead of Nova'shttps://github.com/remodlai/Nova/pull/27188)
from lmcache.utils import _lmcache_nvtx_annotate, cdiv, round_down
from lmcache.v1.cache_engine import LMCacheEngineBuilder

# FIXME(Jiayi): temporarily comment this out
# from lmcache_nova.blend_adapter import remove_request_id_indices
...
NOVA_CACHE_CONFIG: Optional[CacheConfig] = None
NOVA_MODEL_CONFIG: Optional[ModelConfig] = None
NOVA_PARALLEL_CONFIG: Optional[ParallelConfig] = None
NOVA_SCHEDULER_CONFIG: Optional[SchedulerConfig] = None
...
def lmcache_should_store(
    ...
    # `lmcache_store`
    # We use this dirty fix now as we don't want to modify the nova
...
def lmcache_store_kv(
    ...
    for seq_group_idx, seq_group in enumerate(seq_group_list):
        for seqid, seq_data in seq_group.seq_data.items():
            ...
            # TODO (Jiayi): can chunk prefill and nova prefix
            ...
            nova_num_required_tokens = (
                query_start_loc[seq_data_idx + 1] - query_start_loc[seq_data_idx]
            ).item()
            assert isinstance(nova_num_required_tokens, int)

            start_pos = next_start_pos
            end_pos = start_pos + nova_num_required_tokens
            next_start_pos = end_pos

            nova_num_computed_tokens = seq_len - nova_num_required_tokens
            if nova_num_computed_tokens > 0:
                if skip_leading_tokens >= nova_num_computed_tokens:
                    ...
                    slot_mapping_req_full[nova_num_computed_tokens:] = slot_mapping[
                        start_pos:end_pos
                    ]
                else:
                    # NOTE(Jiayi): the cache is stored even if it's in nova
                    # as long as it's not in lmc
                    assert block_tables is not None
                    block_table_full = block_tables[seq_group_idx]
                    nova_block_size = cache_config.block_size
                    ...
                    slot_mapping_req_full = (
                        nova_block_size
                        * block_table_full.repeat_interleave(nova_block_size)
                        + indices
                    )
...
def lmcache_retrieve_kv(
    ...
    for seq_group in seq_group_list:
        ...
        for seq_id in seq_ids:
            ...
            # number of tokens already computed by nova
            # (e.g., chunk prefill, prefix caching)
            nova_num_computed_tokens = total_seq_len - nova_num_required_tokens

            # NOTE: No need to retrieve from lmc if the current sequence is
            # in DECODE stage
            if retrieve_status[idx] == RetrieveStatus.NONE:
                assert nova_num_required_tokens == 1
                total_seq_len = seq_lens[idx]
                num_computed_tokens_list.append(nova_num_computed_tokens)
            ...
            if nova_num_required_tokens < lmc_chunk_size:
                num_computed_tokens_list.append(nova_num_computed_tokens)
                lmc_num_computed_tokens_list.append(0)
                idx += 1
                num_request_not_found += 1
            ...
            # from lmc. Tokens computed in nova already should be skipped
            token_mask = torch.ones_like(full_token_tensor, dtype=torch.bool)
            nova_num_computed_tokens_align = (
                nova_num_computed_tokens // lmc_chunk_size * lmc_chunk_size
            )
            token_mask[:nova_num_computed_tokens_align] = False
            ...
            lmc_num_computed_tokens = max(
                ...
                - (nova_num_computed_tokens - nova_num_computed_tokens_align),
                0,
            )

            assert isinstance(lmc_num_computed_tokens, int)

            # total number of computed tokens (nova + lmc)
            num_computed_tokens = nova_num_computed_tokens + lmc_num_computed_tokens
...
def build_partial_prefill_input(
    ...
    for idx in range(len(full_tokens_list)):
        ...
        if len(model_input.attn_metadata.block_tables[idx]) > 0:
            ...
            nova_block_size = cache_config.block_size
            rebuilt_block_table = (
                slot_mapping_req[::nova_block_size].to(torch.int32) // nova_block_size
            )
    ...
    # Third Party
    from nova.worker.model_runner import ModelInputForGPUWithSamplingMetadata
...
def build_mla_params(
    ...
    assert NOVA_CACHE_CONFIG is not None
    assert NOVA_MODEL_CONFIG is not None
    assert NOVA_SCHEDULER_CONFIG is not None
    assert NOVA_PARALLEL_CONFIG is not None
    ...
    context_chunk_workspace_size = min(
        ...
        max(
            8 * NOVA_MODEL_CONFIG.max_model_len,
            4 * NOVA_SCHEDULER_CONFIG.max_num_seqs * NOVA_CACHE_CONFIG.block_size,
        ),
    ...
    if (
        ...
        max_context_chunk = round_down(max_context_chunk, NOVA_CACHE_CONFIG.block_size)
    ...
    if attention_mata.context_chunk_workspace is None:
        attention_mata.context_chunk_workspace = torch.empty(
            (context_chunk_workspace_size, NOVA_MODEL_CONFIG.get_head_size()),
            dtype=NOVA_MODEL_CONFIG.dtype,
            device=device,
        )
    ...
    if attention_mata.num_decode_tokens > 0:
        # Third Party
        from nova.core.attention.ops.flashmla import get_mla_metadata

        num_q_heads = NOVA_MODEL_CONFIG.get_num_attention_heads(NOVA_PARALLEL_CONFIG)
        (
